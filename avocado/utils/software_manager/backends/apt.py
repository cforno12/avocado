...
class AptBackend(DpkgBackend):
    ...
    def __init__(self, session=None):
        ...
        executable = utils_path.find_command('apt-get', session=session)
    ...
    def add_repo(self, repo):
        ...
        def _get_repo_file_contents():
            if self.session:
                ...
                try:
                    ...
                    log.debug('Exception: %s', self.repo_file_path)
                    return None
        ...
        if self.session:
            if not self.session.cmd('test -f %s' % self.repo_file_path).exit_status == 0:
                _add_repo_file()
                return True
        elif not os.path.isfile(self.repo_file_path):
            _add_repo_file()
            return True
    ...
    def provides(self, name):
        ...
        """
        try:
            command = utils_path.find_command('apt-file', session=session)
        ...
        try:
            if self.build_dep(name):
                if self.session:
                    ...
                    dirs = self.session.cmd('ls').stdout_text.split()
                    for subdir in dirs:
